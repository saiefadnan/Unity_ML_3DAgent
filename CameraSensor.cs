...
namespace Unity.MLAgents.Sensors
{
    ...
    public class CameraSensor : ISensor, IBuiltInSensor, IDisposable
    {
        ...
        /// </summary>
        /// <returns>The <see cref="ObservationSpec"/>.</returns>
        ...
        public void Update()
        {
            // Recreate the texture if it was destroyed (e.g. after scene reload or Dispose was called)
            if (m_Texture == null)
            {
            ...
            // if (m_Grayscale)
            // {
            //     // Create a true grayscale texture for debug saving
            //     Texture2D grayTex = new Texture2D(m_Width, m_Height, TextureFormat.RGB24, false);
            //     Color[] pixels = m_Texture.GetPixels();
            //     for (int i = 0; i < pixels.Length; i++)
            //     {
            //         float lum = pixels[i].grayscale; // Unity built-in luminance
            //         pixels[i] = new Color(lum, lum, lum);
            //     }
            //     grayTex.SetPixels(pixels);
            //     grayTex.Apply();
            //     System.IO.File.WriteAllBytes("camera_debug_gray.png", grayTex.EncodeToPNG());
            //     if (Application.isPlaying)
            //         UnityEngine.Object.Destroy(grayTex);
            //     else
            //         UnityEngine.Object.DestroyImmediate(grayTex);
            // }
            // else
            // {
            //     System.IO.File.WriteAllBytes("camera_debug.png", m_Texture.EncodeToPNG());
            // }
        }
        ...
        {
            // Guard: camera must be assigned and not destroyed
            if (obsCamera == null)
            {
                Debug.LogError("CameraSensor: Camera is null in ObservationToTexture. " +
                               "Make sure a Camera is assigned to the sensor and has not been destroyed.");
                return;
            }
            ...
            if (texture2D == null)
            {
                Debug.LogError("CameraSensor: texture2D is null in ObservationToTexture.");
                return;
            }
            ...
            texture2D.Apply(); // Upload pixel data to GPU; required after ReadPixels
