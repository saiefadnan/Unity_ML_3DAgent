...
public class DroneController : Agent
{
    ...
    public float shortestPath = 0f;        // made public for LogTestRun access
    ...
    public override void Initialize()
    {
        ...
        {
            logFilePath = Path.Combine(Application.dataPath, "..", "3DMulti_Test_Results.csv");
            if (!File.Exists(logFilePath))
            {
                try
                {
                    // ── UPDATED HEADER to match 2D multi columns ──────────────
    ...
    private void LogTestRun()
    {
        ...
        try
        {
            ...
            DroneController[] allAgents = FindObjectsOfType<DroneController>();
            System.Array.Sort(allAgents, (a, b) => a.agentIndex.CompareTo(b.agentIndex));

            string agentGoalsStr      = "";
            string agentHPsStr        = "";
            string agentEfficiencyStr = "";
            int    totalSteps         = 0;

            foreach (var agent in allAgents)
            {
                // Per-agent path efficiency
                ...
                totalSteps          = Mathf.Max(totalSteps, agent.stepCount);
            }
            ...
            int teamRescued  = envConfig.TotalVictimsFound();
            int totalVictims = envConfig.victimCount;
            int teamExplored = envConfig.teamExploredCells.Count;
